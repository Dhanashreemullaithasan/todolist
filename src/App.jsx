...
function App (){
  ...
  const handleupdateTask=(e)=>{
    if(confirm("Do you want update task")){
      console.log(e.target.id);
      let temp = toDoList.filter((v,i) => i === parseInt(e.target.id));
      setToDoList(temp);
    }
...

    {
      ...
      toDoList.length > 0 && toDoList.map((v,i) =>{
        return(
          <div key={i}>
            <span>{v}</span>
            <span><button id={i} onClick={handlecloseTask}>Remove</button></span>
            <span><button id={i} onClick={handleupdateTask}>Update</button></span>
          </div>
        )
      })
    }
